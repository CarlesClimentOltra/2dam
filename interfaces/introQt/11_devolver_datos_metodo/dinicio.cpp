...
void DInicio::slotAceptarTransferencia() {
	...
	QString cantidad = QString::number(dTransf->getCantidad());
	QString cuenta = dTransf->getCuenta();
	QString stringCantidad("Cantidad ");
	QString stringCuenta(" Cuenta: ");
	bool aceptar;

	lResultado->setText("Transferencia aceptada");
	lDatos->setText(stringCantidad + cantidad + stringCuenta + cuenta);
	
	aceptar = dTransf->getInfo(cuenta, cantidad);
