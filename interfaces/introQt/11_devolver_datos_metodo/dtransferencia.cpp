...
void DTransferencia::slotActualizarBoton(const QString &cadena) {
...
void DTransferencia::slotAceptar() {
	...
	emit senyalTransferenciaOrdenada(cantidad);
	emit senyalTransferenciaCompleta(cmbDestino->currentText(), cantidad);
	accept();
...
float DTransferencia::getCantidad() {
	...
	return leCantidad->text().toFloat();

}

QString DTransferencia::getCuenta() {

	return cmbDestino->currentText();

}

bool DTransferencia::getInfo(QString &cuenta, float &cant) {
